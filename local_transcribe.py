...
def transcribe_file(model, srt, plain, file):
    ...
    result = model.transcribe(
        str(input_file_path), fp16=False, verbose=False, language="en"
    )
    ...
    for filename in os.listdir(input_folder):
        if filename.endswith(".mp4"):
            ...
            if transcribe_flag:
                ...
                initial_srt_path = os.path.join(
                    crew_output_folder, f"{os.path.splitext(filename)[0]}.srt"
                )

            if wait_for_file(initial_srt_path):
                whisper_output_dir = "whisper_output"
                srt_files = [
                    f for f in os.listdir(whisper_output_dir) if f.endswith(".srt")
                ]
                txt_files = [
                    f for f in os.listdir(whisper_output_dir) if f.endswith(".txt")
                ]
                ...
                if srt_files and txt_files:
                    ...
                    for srt_filename in sorted(os.listdir(crew_output_folder)):
                        if srt_filename.startswith(
                            "new_file_return_subtitles"
                        ) and srt_filename.endswith(".srt"):
                            subtitle_file_path = os.path.join(
                                crew_output_folder, srt_filename
                            )
                else:
                    logging.error(
                        "No .srt or .txt files found in the whisper_output directory."
                    )
            else:
                logging.error(
                    f"Failed to verify the readiness of subtitles file: {initial_srt_path}"
                )
